...
class Analyzer implements Runnable {
    ...
    /**
     * Monitoring scheduler for analyzing reporting.
     */
    protected MonitoringScheduler scheduler;
    /**
     * Only Report Errors.
     */
     ...
     *
     * @param scheduler scheduler for reporting
     ...
     */
    Analyzer(MonitoringScheduler scheduler, Address address, TextChannel channel, boolean onlyError) {
    ...
    private void analyzeMinecraftAddress() {
        ...
        if (minecraftPing.isOnline() && !onlyError) {
            ...
            channel.sendMessage(builder.build()).queue();
            if (!scheduler.markedAsUnreachable(channel.getGuild().getIdLong(), address)) {
                channel.sendMessage("@here").queue();
            }
            scheduler.markAsUnreachable(channel.getGuild().getIdLong(), address);
        }
