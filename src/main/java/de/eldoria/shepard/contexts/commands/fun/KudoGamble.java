...
public class KudoGamble extends Command {
    ...
    private class SlotMachine {
        ...
        SlotMachine(int slotAmount, int slotvalues) {
            this.slotvalues = slotvalues;

            for (int i = 0; i < slotAmount; i++) {
                slots.add(rand.nextInt(slotvalues));
            }
        }
        ...
        class Slot {
            ...
            public Slot(int value, int multiplier, int minMultiplier, int minSlotsNeeded, int probability) {
                this.value = value;
                this.multiplier = multiplier;
                this.minMultiplier = minMultiplier;
                this.minSlotsNeeded = minSlotsNeeded;
                this.probability = probability;
            }

            /**
             * Get the multiplier for this slot, based on max slots and the presend slots of this type.
             *
             * @param slotsPresent
             * @param slotsMax
             * @return
             */
            public float getMultiplier(int slotsPresent, int slotsMax) {
                if (slotsPresent < minSlotsNeeded) {
                    return 0;
                }
                if (slotsPresent == slotsMax) {
                    return multiplier;
                }
                if (slotsPresent == minSlotsNeeded) {
                    return minMultiplier;
                }
                int diffMultiplier = multiplier - minMultiplier;
                int diffSlots = slotsMax - minSlotsNeeded;

                int multiplierPerSlot = diffMultiplier / diffSlots;

                int diffSlotsPresent = slotsPresent - minSlotsNeeded;

                return minMultiplier + diffSlotsPresent * multiplierPerSlot;
            }
